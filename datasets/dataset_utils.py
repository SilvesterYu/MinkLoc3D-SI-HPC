            ...
            if version == 'MinkLoc3D':
                ...
                for e in batch:
                    ...
                    spherical_e = torch.tensor(to_spherical(
                        e.numpy(), dataset_name), dtype=torch.float)
                    # -- changed
                    c, f = ME.utils.sparse_quantize(coordinates=spherical_e[:, :6],features=spherical_e[:, 6].reshape([-1, 1]), quantization_size=mink_quantization_size)
                    """
                    # -- original below -- #
...
def to_spherical(points, dataset_name):
    ...
    for point in points:
        ...
        if point.shape[-1] == 4:
            ...
            # spherical_points.append([r, theta, phi, r, theta, phi, point[3]])
            # -- original below -- #
